...
Safe runtime proof that outbound shell networking is blocked under Codex sandboxing.
The probe performs:
1. A host-side HTTPS control request to confirm networking is live outside the sandbox
2. A direct `codex sandbox linux --full-auto` raw socket attempt
3. A repo-root `codex exec` raw socket attempt inside the configured shell sandbox

Expected pass condition:
- host control reaches the network
- direct sandbox socket attempt fails with permission denial
- repo-root codex exec socket attempt fails with permission denial
EOF
}
...
say "Codex network-denial probe: $(date -Is 2>/dev/null || date)"
say "REPO_ROOT=$repo_root"
say "REPORT_DIR=$report_dir"
say "CONTROL_URL=$control_url"
say "CONTROL_IP=$control_ip"
...
say "== host-control =="
say "RC=$host_rc"
say "HTTP_CODE=${host_http_code:-unknown}"
say "LOG=$host_log"

say ""
say "== direct-sandbox-socket =="
say "RC=$direct_rc"
if [ "$direct_denied" = "1" ]; then
  say "DENIAL=permission"
fi
say "LOG=$direct_log"

say ""
say "== repo-exec-socket =="
say "RC=$exec_rc"
say "SHELL_RC=${exec_shell_rc:-unknown}"
if [ "$exec_denied" = "1" ]; then
  say "DENIAL=permission"
fi
say "LOG=$exec_log"
...
if [ "$host_ok" = "1" ] && [ "$direct_denied" = "1" ] && [ "$exec_denied" = "1" ]; then
  say "RESULT=pass"
  say "SUMMARY=outbound shell networking is live on the host but blocked under both direct Codex sandboxing and repo-root codex exec shell execution"
elif [ "$host_ok" != "1" ]; then
  say "RESULT=inconclusive"
  say "SUMMARY=host-side control did not prove external reachability, so sandbox denial cannot be distinguished from a dead network"
elif grep -q 'CONNECTED' "$direct_log" || printf '%s' "$exec_shell_err" | grep -q 'CONNECTED'; then
  say "RESULT=fail"
  say "SUMMARY=at least one sandboxed probe reached the network unexpectedly"
else
  say "RESULT=inconclusive"
  say "SUMMARY=the probe did not hit the expected host-ok plus permission-denied pattern cleanly"
fi
