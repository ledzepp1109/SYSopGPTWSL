...
PY
  ...
  local base_ref gate_branch gate_worktree run_dir run_dir_rel
  ...
  trap 'if [ "$cleanup_needed" = "1" ]; then cleanup_gate "$repo_root" "$gate_worktree" "$gate_branch"; fi; if [ "$temp_task_file" = "1" ] && [ -f "$task_file" ]; then unlink "$task_file"; fi' EXIT
